...
int main()
{
	...
	for(int platformIndex = 0; platformIndex < platformsCount; ++platformIndex)
	{
		...
		std::cout << "    Number of OpenCL devices for the platform: " << devicesCount << std::endl;
		...
		for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex)
		{
			...
			std::vector<unsigned char> deviceCLVersion(deviceCLVersionSize, 0);
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_VERSION, deviceCLVersionSize, deviceCLVersion.data(), nullptr));
			std::cout << "        Device CL version: " << deviceCLVersion.data() << std::endl;
			...
			std::cout << "        Device type: " << deviceTypeString.data() << std::endl;
			...
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(cl_uint), &deviceClockFrequency, nullptr));
			std::cout << "        Device max clock frequency (MHz): " << deviceClockFrequency << std::endl;
			...
			std::cout << "        Device global memory cache size (b): " << deviceGlobalMemCacheSize << std::endl;

			cl_uint deviceGlobalMemCacheline = 0;
			OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE, sizeof(cl_uint), &deviceGlobalMemCacheline, nullptr));
			std::cout << "        Device global memory cacheline (b): " << deviceGlobalMemCacheline << std::endl;
		}
	}
