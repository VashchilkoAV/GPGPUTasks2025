                                ...
                                unsigned int k)
{
    ...
    for (uint tile_inner = 0; tile_inner < tile_count; tile_inner++) {
        ...
        for (uint inner_num = 0; inner_num < GROUP_SIZE_X; inner_num++) {
            ...
            value += a[(tile_y * GROUP_SIZE_X + local_row) * k + (tile_inner * GROUP_SIZE_X + inner_num)] * local_data[inner_num * GROUP_SIZE_X + local_col];
            // }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    ...
    c[(tile_y * GROUP_SIZE_X + local_row) * w + (tile_x * GROUP_SIZE_X + local_col)] = value;
