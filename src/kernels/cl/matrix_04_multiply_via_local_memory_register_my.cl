                                ...
                                unsigned int k)
{
    ...
    for (uint tile_inner = 0; tile_inner < tile_count; tile_inner++) {
        ...
        for (uint reg_num = 0; reg_num < REGISTER_GROUP_SIZE_X; reg_num++) {
            local_dataA[local_row * local_w + local_col + reg_num * THREADS_PER_TILE_REGISTER] = a[row * k + tile_col + reg_num * THREADS_PER_TILE_REGISTER];
            local_dataB[local_row * local_w + local_col + reg_num * THREADS_PER_TILE_REGISTER] = b[tile_row * w + col + reg_num * THREADS_PER_TILE_REGISTER];  
        }
